...
Pages, one file each under `site/src/content/docs/`: install (brew, make), use (every flag, exit codes), redaction (rule table, `--rules` format, the injection note), ledger (location, JSON line, `--migrate`), video, pdf, claude-code (plugin install, hook behaviour, `cheapshot allow`), mcp (three tools, one resource), the two research notes as reference pages, and a credits page. The credits page (Ryan, 2026-09-12) links every dependency, tool, service, prior-art project, and source the project used: Apple frameworks, ffmpeg, Starlight and its theme, superpowers, the OCR and redaction neighbours from the market scan, every URL in the research notes, and the people or projects whose ideas were borrowed. It is seeded now as `docs/credits.md` and every later phase adds to it when it adds a dependency. The README shrinks to install, one example per feature, and a link.
