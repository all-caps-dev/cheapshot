...
hits=$(git ls-files | grep -v -e '^docs/product-brief.md$' -e '^docs/superpowers/' -e '^scripts/check-license\.sh$' | xargs grep -l -i -e 'Business Source' -e 'BUSL' 2>/dev/null || true)
